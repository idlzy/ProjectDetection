...
def main():
    parser = argparse.ArgumentParser(description="Validate a checkpoint")
    parser.add_argument("--config", required=True); parser.add_argument("--checkpoint", required=True)
    parser.add_argument("--set", action="append", default=[], dest="overrides")
    args = parser.parse_args()
    print(json.dumps(evaluate_checkpoint(load_config(args.config, args.overrides), args.checkpoint, "val"), ensure_ascii=False, indent=2))
